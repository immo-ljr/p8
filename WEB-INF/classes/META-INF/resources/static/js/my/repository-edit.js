$(document).ready(function () {
	$.ajax({
        type: 'POST',
        async: false,
        url: 'kettle/kRepositoryType/getType',
        data: {},
	...
	$.ajax({
        type: 'POST',
        async: false,
        url: 'kettle/kRepositoryType/getAccess',
        data: {},
        success: function (datas) {
        	var data = datas.data;
        	for (var i=0; i<data.length; i++){
...
var testConnection = function(){
	...
	$.ajax({
        type: 'POST',
        async: false,
        url:'kettle/kRepositoryType/ckeck',
        data: kRepository,
        processData:false,
        contentType:false,
...
var reset = function(){
	var id = $("#repositoryId").val();
	$.ajax({
        type: 'POST',
        async: true,
        url: 'kettle/kRepository/getKRepository',
        data: {
        	id : id
        },
        success: function (data) {
...
$().ready(function () {
    ...
    $("#RepositoryForm").validate({
        ...
        submitHandler:function(form){        	
        	//做判断
        	if (testConnection()){
        		$.post("kettle/kRepository/update", decodeURIComponent($(form).serialize(),true), function(data){
        			if(data.httpStatus == "200"){
        				layer.msg('更新成功',{
                			time: 2000,
                			icon: 6
                		...
                		setTimeout(function(){
                			location.href = "index/listRepositoryUI";
                		},2000);
        			}else {
