        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
            	<div class="col-sm-6 float-left">	
	            	<a href="index/transMonitorUI" class="btn btn-w-m btn-info" type="button">
                ...
                <table id="transRecordList" data-toggle="table"
					data-url="kettle/kTransRecord/getListPage"
	...
	<script>
		$(document).ready(function () {
			...
			$.ajax({
		        type: 'POST',
		        async: false,
		        url: 'kettle/kTrans/getAllList',
		        data: {
		        },
		        success: function (datas) {
		        	var data = datas.data;
		        	for (var i=0; i<data.length; i++){
		        		$("#transId").append('<option value="' + data[i].id + '">' + data[i].transName + '</option>');
		        	}
		        },
		        error: function () {
		            alert("请求失败！请刷新页面重试");
	    ...
	    function actionFormatter(value, row, index) {
	    	if(row.recordStatus=='2'){  //失败 ,展示查看错误日志的按钮
	    		return ['<a class="btn btn-primary btn-xs" id="view" type="button"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;查看日志</a>',
	        			'&nbsp;&nbsp;',
	        			'<a class="btn btn-primary btn-xs" id="download" type="button"><i class="fa fa-download" aria-hidden="true"></i>&nbsp;下载日志</a>',
	        			'&nbsp;&nbsp;',
	        			'<a class="btn btn-primary btn-xs" id="detail" type="button"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;查看明细</a>',
	        			'&nbsp;&nbsp;',
	        			'<a class="btn btn-primary btn-xs" id="error_detail" type="button"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;查看错误日志</a>'].join('');
	    	}else{
	    		return ['<a class="btn btn-primary btn-xs" id="view" type="button"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;查看日志</a>',
	        			'&nbsp;&nbsp;',
	        			'<a class="btn btn-primary btn-xs" id="download" type="button"><i class="fa fa-download" aria-hidden="true"></i>&nbsp;下载日志</a>',
	        			'&nbsp;&nbsp;',
	        			'<a class="btn btn-primary btn-xs" id="detail" type="button"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;查看明细</a>'].join('');
	    	}
	    };
	    window.actionEvents = {				
	    		'click #view' : function(e, value, row, index) {
	    			$.ajax({
				        type: 'POST',
				        async: false,
				        url: 'kettle/kTransRecord/getLogContent',
				        data: {
				            "recordId": row.id          
				        },
	    		...
	    		'click #error_detail' : function(e, value, row, index) {
	    			$.ajax({
				        type: 'POST',
				        async: false,
				        url: 'kettle/kTransRecord/getErrorLogContent',
				        data: {
				            "recordId": row.id          
				        },
				        success: function (data) {
	    			...
	    			layer.confirm('确定下载该日志记录？', {
	    				...
	    				function(index){
	                		...
	                		var form = $('<form>');
	        	            	...
	        	            	form.attr('action', 'kettle/kTransRecord/download');  
		        	        var $recordId = $('<input>');   
	    		...
	    		'click #detail' : function(e, value, row, index) {
	    			...
	    			location.href="index/transLogListUI?recordId=" + row.id+"&transId="+$transId+"&startDate="+$startDate+"&endDate="+$endDate;    			
	    		},
	    	};
		    ...
		    function queryParams(params) {
		    	...
		    	var $startDate = $("#startDateId").val();
		    	var $endDate =$("#endDateId").val();
		    	var temp = {limit: 10, offset: params.offset/10+1, transId: $transId, startDate:$startDate, endDate:$endDate};
		        return temp;
		    };
		    		    
		    function search(){
		    	$('#transRecordList').bootstrapTable('refresh', "kettle/kTransRecord/getListPage");
		    };
