				...
				<div class="widget style1 navy-bg">
					<div class="row">
						...
						<div class="col-sm-8 text-right" style="font-size: 20px">
							<span> 总WS任务数 </span>
							<h2 class="font-bold" id="allWs"></h2>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="ibox float-e-margins">
			...
			<div class="ibox-content">
				...
				<table id="transMonitorList" data-toggle="table"
					data-url="kettle/kWebserviceMonitor/getListPage"
					...
					<thead>
						<tr>
							...
							<th data-field="monitorWebservice" data-formatter="MonitorWebserviceFormatter">任务名称</th>
							<th data-field="monitorFunction">WS方法名</th>
							<th data-field="monitorSuccess">执行成功次数</th>
							<th data-field="monitorFail">执行失败次数</th>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type : 'POST',
				async : false,
				url : 'kettle/kWebserviceMonitor/getAllMonitorTransCount',
				data : {},
			...
			$.ajax({
				type : 'POST',
				async : false,
				url : 'kettle/kWebserviceMonitor/getAllSuccessCount',
				data : {},
				success : function(datas) {
					var data = datas.data;
					console.log(data);
			...
			$.ajax({
				type : 'POST',
				async : false,
				url : 'kettle/kWebserviceMonitor/getAllFailCount',
				data : {},
				success : function(datas) {
					var data = datas.data;
					$("#allFail").text(data);
		...
		function MonitorWebserviceFormatter(value, row, index) {
			var Monitor = "";
			$.ajax({
				type : 'POST',
				async : false,
		...
		window.actionEvents = {
			'click #viewDetail' : function(e, value, row, index) {
				location.href = "kettle/wsRecord/recordListUI?wsId="
						+ row.monitorWebservice + "&startDate=&endDate=";
			},
		};
